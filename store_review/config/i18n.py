...
STRINGS: dict[str, dict[str, str]] = {
    ...
    "dash.summary_narrative_positive": {
        "tr": "Bu hızlı duygu özeti, {total} sınıflandırılmış görüşe dayanır ve tek tek yorumdan alıntı yapmaz. Yaklaşık dağılım: olumlu {pos_p}% ({pos} görüş), olumsuz {neg_p}% ({neg} görüş), nötr veya kararsız bant {neu_p}% ({neu} görüş). Bu kesitte kolektif okuma belirgin şekilde destekleyicidir; çoğunluk kullanıcıları deneyimi genel olarak olumlu yorumlar. Kalan olumsuz pay ise sürtünme alanlarına—kararlılık, performans, ilk kullanım veya beklenti yönetimi—işaret eder ve ürün iletişiminde izlenmelidir. Tüm kaynak dillerdeki metinler aynı çerçevede puanlandığından anlatı yalnızca toplu payları yansıtır, kelimesi kelimesine kullanıcı dilini yansıtmaz.",
        "en": "This quick sentiment summary is based on {total} classified opinions and does not quote individual reviews. The split is roughly {pos_p}% positive ({pos} opinions), {neg_p}% negative ({neg} opinions), and {neu_p}% in a neutral or ambivalent band ({neu} opinions). At this snapshot the collective reading is clearly supportive: most users are inclined to view the experience favorably overall. The remaining negative share still signals pockets of friction—reliability, performance, onboarding, or expectations management—worth monitoring in product and communications work. Texts in all source languages were scored in the same framework, so the narrative reflects aggregate share only, not verbatim user language.",
        "es": "Este resumen rápido de sentimiento se basa en {total} opiniones clasificadas y no cita reseñas individuales. La proporción es aproximadamente {pos_p}% positivas ({pos} opiniones), {neg_p}% negativas ({neg} opiniones) y {neu_p}% en una franja neutra o ambivalente ({neu} opiniones). En este corte temporal la lectura colectiva es claramente favorable: la mayoría tiende a valorar la experiencia de modo globalmente positivo. La parte negativa restante sigue señalando fricciones—fiabilidad, rendimiento, incorporación o gestión de expectativas—que conviene vigilar en producto y comunicación. Los textos en todos los idiomas se puntuaron con el mismo marco, por lo que el relato refleja solo la proporción agregada, no el lenguaje literal del usuario.",
        "de": "Diese kompakte Stimmungsübersicht stützt sich auf {total} klassifizierte Meinungen und zitiert keine Einzelrezensionen. Ungefähr {pos_p}% sind positiv ({pos} Meinungen), {neg_p}% negativ ({neg} Meinungen) und {neu_p}% neutral oder zwiespältig ({neu} Meinungen). In diesem Ausschnitt ist die kollektive Lesart deutlich unterstützend: die Mehrheit bewertet die Erfahrung insgesamt eher positiv. Der verbleibende negative Anteil weist dennoch auf Reibung hin—Zuverlässigkeit, Performance, Onboarding oder Erwartungsmanagement—die Produkt- und Kommunikationsarbeit begleiten sollten. Texte aller Ausgangssprachen wurden im gleichen Rahmen bewertet; die Darstellung spiegelt daher nur aggregierte Anteile, nicht wörtliche Nutzerformulierungen.",
        "fr": "Ce résumé rapide de sentiment repose sur {total} opinions classées et ne cite pas d’avis individuels. La répartition est d’environ {pos_p}% positives ({pos} opinions), {neg_p}% négatives ({neg} opinions) et {neu_p}% dans une zone neutre ou ambivalente ({neu} opinions). À cette fenêtre, la lecture collective est nettement favorable : la majorité tend à juger l’expérience globalement positive. La part négative restante signale toutefois des frictions—fiabilité, performance, onboarding ou gestion des attentes—à suivre dans le produit et la communication. Les textes dans toutes les langues sources ont été notés dans le même cadre ; le récit reflète donc uniquement les parts agrégées, pas le langage verbatim des utilisateurs.",
        "ar": "يعتمد هذا الملخص السريع للمشاعر على {total} رأي مصنف ولا يقتبس مراجعات فردية. التقسيم تقريبًا {pos_p}% إيجابي ({pos} رأي) و{neg_p}% سلبي ({neg} رأي) و{neu_p}% في نطاق محايد أو متردد ({neu} رأي). في هذه اللقطة القراءة الجماعية داعمة بوضوح: غالبية المستخدمين يميلون إلى تقييم التجربة بإيجابية إجمالية. لا تزال الحصة السلبية تشير إلى احتكاكات—الاعتمادية، الأداء، الإدماج أو إدارة التوقعات—تستحق المتابعة في العمل على المنتج والتواصل. جُمِعت النصوص بكل اللغات ضمن إطار واحد، لذا يعكس السرد حصص التجميع فقط لا صياغات المستخدم حرفيًا.",
        "zh": "本情绪快览基于 {total} 条已分类观点，不引用单条评论原文。大致比例为：正面约 {pos_p}%（{pos} 条）、负面约 {neg_p}%（{neg} 条）、中性或摇摆约 {neu_p}%（{neu} 条）。在当前样本下，整体倾向明显偏正面：多数用户总体上更愿意给出积极评价。仍存在的负面占比提示在可靠性、性能、上手体验或预期管理等方面可能存在摩擦，需在产品和沟通中持续关注。所有语言的文本均按同一规则打分，因此叙述仅反映汇总占比，而非逐字引用用户措辞。",
        "ru": "Этот краткий сводный обзор опирается на {total} классифицированных мнений и не цитирует отдельные отзывы. Доля примерно {pos_p}% позитивных ({pos} мнений), {neg_p}% негативных ({neg} мнений) и {neu_p}% в нейтральной или смешанной зоне ({neu} мнений). На этом срезе коллективный сигнал явно поддерживающий: большинство пользователей в целом склонны оценивать опыт благоприятно. Оставшаяся негативная доля всё же указывает на трения — надёжность, производительность, онбординг или управление ожиданиями — за которыми стоит следить в продукте и коммуникациях. Тексты на всех языках оценивались в одной схеме, поэтому вывод отражает только агрегированные доли, а не дословные формулировки.",
        "pt": "Este resumo rápido de sentimento baseia-se em {total} opiniões classificadas e não cita avaliações individuais. A divisão é cerca de {pos_p}% positivas ({pos} opiniões), {neg_p}% negativas ({neg} opiniões) e {neu_p}% numa faixa neutra ou ambivalente ({neu} opiniões). Neste recorte a leitura coletiva é claramente favorável: a maioria tende a ver a experiência de modo globalmente positivo. A parcela negativa remanescente ainda sinaliza atritos—confiabilidade, desempenho, integração ou gestão de expectativas—que merecem acompanhamento em produto e comunicação. Textos em todos os idiomas foram pontuados no mesmo quadro, portanto a narrativa reflete apenas a proporção agregada, não a linguagem literal do usuário.",
    },
    "dash.summary_narrative_negative": {
        "tr": "Bu özet {total} sınıflandırılmış görüşe dayanır; alıntı içermez. Yaklaşık olarak olumsuz {neg_p}% ({neg} görüş), olumlu {pos_p}% ({pos} görüş), nötr veya karışık {neu_p}% ({neu} görüş). Toplam sinyal ciddi biçimde gergindir; kullanıcı kitlesinin geniş bir kesimi memnuniyetsizlik veya kaygı ifade eder. Olumlu sesler güçlü yönleri gösterebilir ancak bu dilimde onlar baskın eleştirel kütleyi aşmaz. Öncelikler kararlılık, hata yönetimi, gelir modelinin algılanan adaleti ve net değer sunumunda yoğunlaşmalıdır. Model tüm dillere eşit yaklaştığı için sonuçlar genel dağılım seviyesindedir, bireysel ifadeleri yansıtmaz.",
        "en": "This summary draws on {total} classified opinions without quoting review text. The mix is roughly {neg_p}% negative ({neg} opinions), {pos_p}% positive ({pos} opinions), and {neu_p}% neutral or mixed ({neu} opinions). The aggregate signal is decisively strained: a large part of the user base currently expresses dissatisfaction or concern. Positive voices may still highlight strengths, but they do not outweigh the dominant critical mass in this slice. Priorities should emphasize stability, bug handling, perceived fairness of monetization, and clearer value delivery. Because the model treats every language uniformly, conclusions stay at the level of overall sentiment distribution rather than localized phrasing.",
        "es": "Este resumen se apoya en {total} opiniones clasificadas sin citar el texto de las reseñas. La mezcla es aproximadamente {neg_p}% negativas ({neg} opiniones), {pos_p}% positivas ({pos} opiniones) y {neu_p}% neutras o mixtas ({neu} opiniones). La señal agregada es claramente tensa: una parte importante de la base de usuarios expresa insatisfacción o preocupación. Las voces positivas pueden señalar fortalezas, pero no superan la masa crítica dominante en este recorte. Las prioridades deben enfatizar estabilidad, corrección de errores, equidad percibida de la monetización y una entrega de valor más clara. Al tratar todos los idiomas por igual, las conclusiones permanecen en el nivel de la distribución global del sentimiento, no en formulaciones locales.",
        "de": "Diese Zusammenfassung basiert auf {total} klassifizierten Meinungen ohne Zitate aus Rezensionstexten. Ungefähr {neg_p}% sind negativ ({neg} Meinungen), {pos_p}% positiv ({pos} Meinungen) und {neu_p}% neutral oder gemischt ({neu} Meinungen). Das Gesamtsignal ist deutlich angespannt: ein großer Teil der Nutzerbasis äußert Unzufriedenheit oder Sorge. Positive Stimmen können Stärken zeigen, überwiegen in diesem Ausschnitt aber nicht die dominante kritische Masse. Prioritäten sollten Stabilität, Fehlerbehebung, wahrgenommene Fairness der Monetarisierung und klarere Wertlieferung betonen. Da das Modell alle Sprachen gleich behandelt, bleiben Schlussfolgerungen auf der Ebene der Gesamtverteilung, nicht lokaler Formulierungen.",
        "fr": "Ce résumé s’appuie sur {total} opinions classées sans citer le texte des avis. La répartition est d’environ {neg_p}% négatives ({neg} opinions), {pos_p}% positives ({pos} opinions) et {neu_p}% neutres ou mixtes ({neu} opinions). Le signal agrégé est nettement tendu : une large part de la base utilisateur exprime de l’insatisfaction ou de l’inquiétude. Les voix positives peuvent souligner des atouts, mais elles ne l’emportent pas sur la masse critique dominante dans cette fenêtre. Les priorités doivent mettre l’accent sur la stabilité, la correction des bugs, l’équité perçue de la monétisation et une promesse de valeur plus claire. Comme le modèle traite toutes les langues de façon uniforme, les conclusions restent au niveau de la distribution globale du sentiment, pas des formulations locales.",
        "ar": "يعتمد هذا الملخص على {total} رأي مصنف دون اقتباس نص المراجعات. المزيج تقريبًا {neg_p}% سلبي ({neg} رأي) و{pos_p}% إيجابي ({pos} رأي) و{neu_p}% محايد أو مختلط ({neu} رأي). الإشارة الإجمالية مشددة بوضوح: جزء كبير من قاعدة المستخدمين يعبّر عن عدم رضا أو قلق. قد تُبرز الأصوات الإيجابية نقاط قوة لكنها لا تطغى على الكتلة النقدية السائدة في هذه القطعة. يجب أن تُركز الأولويات على الاستقرار ومعالجة الأخطاء وعدالة الإيرادات كما يُدركها المستخدمون وتقديم قيمة أوضح. وبما أن النموذج يعامل كل اللغات بالتساوي، تبقى الاستنتاجات على مستوى توزيع المشاعر الإجمالي لا الصياغات المحلية.",
        "zh": "本摘要依据 {total} 条已分类观点，不引用评论原文。大致构成为：负面约 {neg_p}%（{neg} 条）、正面约 {pos_p}%（{pos} 条）、中性或混合约 {neu_p}%（{neu} 条）。整体信号明显偏紧：用户群中相当大比例表达不满或担忧。正面声音仍可能体现优势，但在此样本中不足以压倒主导的批评性多数。优先级应强调稳定性、缺陷修复、变现公平感以及更清晰的价值交付。模型对各语言一视同仁，因此结论停留在总体情绪分布层面，而非局部措辞。",
        "ru": "Этот обзор опирается на {total} классифицированных мнений без цитирования текста отзывов. Соотношение примерно {neg_p}% негативных ({neg} мнений), {pos_p}% позитивных ({pos} мнений) и {neu_p}% нейтральных или смешанных ({neu} мнений). Совокупный сигнал явно напряжён: значительная часть аудитории выражает недовольство или опасение. Позитивные голоса могут подчёркивать сильные стороны, но в этом срезе не перевешивают доминирующую критическую массу. Приоритеты — стабильность, устранение ошибок, воспринимаемая справедливость монетизации и более ясная ценность для пользователя. Модель обрабатывает все языки одинаково, поэтому выводы остаются на уровне общего распределения тональности, а не локальных формулировок.",
        "pt": "Este resumo usa {total} opiniões classificadas sem citar o texto das avaliações. A composição é cerca de {neg_p}% negativas ({neg} opiniões), {pos_p}% positivas ({pos} opiniões) e {neu_p}% neutras ou mistas ({neu} opiniões). O sinal agregado está claramente tenso: grande parte da base expressa insatisfação ou preocupação. Vozes positivas ainda podem destacar pontos fortes, mas não superam a massa crítica dominante neste recorte. Prioridades devem enfatizar estabilidade, correção de bugs, equidade percebida da monetização e entrega de valor mais clara. Como o modelo trata todos os idiomas de forma uniforme, as conclusões permanecem no nível da distribuição geral de sentimento, não de formulações locais.",
    },
    "dash.summary_narrative_mixed": {
        "tr": "Bu birleşik özet {total} sınıflandırılmış görüşü kapsar; bireysel alıntıdan kaçınır. Dağılım kabaca {pos_p}% olumlu ({pos} görüş), {neg_p}% olumsuz ({neg} görüş), {neu_p}% nötr veya karışık ({neu} görüş). Tek bir kutup hâkim değildir; topluluk hem memnuniyeti hem hayal kırıklığını yoğun biçimde bildirir. Yönetim bunu kutuplaşmış beklentiler olarak okumalıdır: bir kesim güçlü değer görürken diğeri engeller veya vaat ile gerçeklik arasındaki kopukluk yaşar. Yol haritaları segment-duyarlı dengelemeden yararlanır. Metin tüm dillerde aynı şekilde birleştirildiğinden öneriler yalnızca oran temelli fikir birliğini yansıtır.",
        "en": "This synthesis covers {total} classified opinions and avoids individual quotations. Sentiment splits about {pos_p}% positive ({pos} opinions), {neg_p}% negative ({neg} opinions), and {neu_p}% neutral or mixed ({neu} opinions). No single pole dominates; the community simultaneously reports satisfaction and frustration at scale. Management should read this as polarized expectations: one segment perceives strong value while another hits blockers or disconnects between promise and reality. Roadmaps benefit from segment-aware balancing rather than assuming one narrative fits all. The text aggregates across languages identically, so guidance reflects ratio-based consensus only.",
        "es": "Esta síntesis abarca {total} opiniones clasificadas y evita citas individuales. El sentimiento se reparte en torno a {pos_p}% positivas ({pos} opiniones), {neg_p}% negativas ({neg} opiniones) y {neu_p}% neutras o mixtas ({neu} opiniones). Ningún polo domina; la comunidad informa a la vez satisfacción y frustración a escala. La dirección debe leerlo como expectativas polarizadas: un segmento percibe mucho valor mientras otro encuentra bloqueos o desajustes entre promesa y realidad. Las hojas de ruta ganan con un equilibrio consciente de segmentos en lugar de asumir una sola narrativa. La agregación es idéntica en todos los idiomas, por lo que la orientación refleja solo un consenso basado en proporciones.",
        "de": "Diese Synthese umfasst {total} klassifizierte Meinungen und vermeidet Einzelzitate. Die Stimmung verteilt sich auf etwa {pos_p}% positiv ({pos} Meinungen), {neg_p}% negativ ({neg} Meinungen) und {neu_p}% neutral oder gemischt ({neu} Meinungen). Kein Pol dominiert; die Community meldet gleichzeitig Zufriedenheit und Frustration in nennenswem Umfang. Management sollte polarisierte Erwartungen lesen: ein Segment sieht starken Nutzen, ein anderes stößt auf Blocker oder Lücken zwischen Versprechen und Realität. Roadmaps profitieren von segmentbewusster Balance statt einer Einheitsnarrative. Die Aggregation erfolgt sprachübergreifend identisch; die Leitlinie spiegelt daher nur konsensbasierte Anteile wider.",
        "fr": "Cette synthèse couvre {total} opinions classées et évite les citations individuelles. Le sentiment se répartit autour de {pos_p}% positives ({pos} opinions), {neg_p}% négatives ({neg} opinions) et {neu_p}% neutres ou mixtes ({neu} opinions). Aucun pôle ne domine ; la communauté signale à la fois satisfaction et frustration à grande échelle. La direction doit lire des attentes polarisées : un segment perçoit une forte valeur tandis qu’un autre rencontre des blocages ou un décalage promesse–réalité. Les feuilles de route gagnent à équiliser par segment plutôt qu’à imposer une seule narration. L’agrégation est identique pour toutes les langues, donc l’orientation reflète uniquement un consensus fondé sur les proportions.",
        "ar": "تغطي هذه الصياغة {total} رأيًا مصنفًا وتتجنب الاقتباسات الفردية. ينقسم المزاج إلى نحو {pos_p}% إيجابي ({pos} رأي) و{neg_p}% سلبي ({neg} رأي) و{neu_p}% محايد أو مختلط ({neu} رأي). لا يهيمن قطب واحد؛ المجتمع يبلغ عن الرضا والإحباط معًا وبشكل واسع. على الإدارة أن تقرأ توقعات متقاطعة: شريحة ترى قيمة قوية وأخرى تواجه عوائق أو فجوة بين الوعد والواقع. تستفيد خرائط الطريق من موازنة واعية بالشرائح بدل افتراض سرد واحد يناسب الجميع. التجميع متطابق عبر اللغات، لذا تعكس التوجيهات إجماعًا نسبيًا فقط.",
        "zh": "本综合解读覆盖 {total} 条已分类观点，避免逐条引用。情绪大致分为：正面约 {pos_p}%（{pos} 条）、负面约 {neg_p}%（{neg} 条）、中性或混合约 {neu_p}%（{neu} 条）。没有单一极占主导；社群同时在大规模报告满意与受挫。管理层应将其读作预期分化：一部分用户感知到强价值，另一部分遇到阻碍或承诺与现实脱节。路线图更适合按细分人群做平衡，而非假设一套叙事适用所有人。各语言以相同方式汇总，因此指引仅反映基于比例的共识。",
        "ru": "Этот синтез охватывает {total} классифицированных мнений и избегает индивидуальных цитат. Тональность делится примерно на {pos_p}% позитивных ({pos} мнений), {neg_p}% негативных ({neg} мнений) и {neu_p}% нейтральных или смешанных ({neu} мнений). Ни один полюс не доминирует; сообщество одновременно сообщает об удовлетворённости и разочаровании в значимом объёме. Руководству стоит читать это как поляризованные ожидания: один сегмент видит сильную ценность, другой упирается в блокеры или разрыв между обещанием и реальностью. Дорожные карты выигрывают от сегментно-осознанного баланса, а не от единой истории для всех. Агрегация по языкам одинакова, поэтому рекомендации отражают лишь долевой консенсус.",
        "pt": "Esta síntese cobre {total} opiniões classificadas e evita citações individuais. O sentimento divide-se cerca de {pos_p}% positivas ({pos} opiniões), {neg_p}% negativas ({neg} opiniões) e {neu_p}% neutras ou mistas ({neu} opiniões). Nenhum polo domina; a comunidade relata satisfação e frustração em paralelo em escala. A gestão deve ler expectativas polarizadas: um segmento percebe forte valor enquanto outro encontra bloqueios ou desalinhamento entre promessa e realidade. Roadmaps ganham com equilíbrio consciente de segmentos em vez de uma narrativa única. A agregação é idêntica entre idiomas, logo a orientação reflete apenas consenso baseado em proporções.",
    },
